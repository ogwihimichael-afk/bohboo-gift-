...
function usePhantom() {
    ...
    for (const ep of endpoints) {
      try {
        ...
        if (d.result?.value !== undefined) {
          return (d.result.value / LAM).toFixed(4);
        }
      } catch {}
    }
    // Fallback: read balance directly from Phantom provider
    try {
      const p = ph();
      if (p?.publicKey) {
        const conn = { getBalance: async () => { return 0; } };
        // Try window.solana's connection if available
      }
    } catch {}
    return "0.0000";
  },[]);
  ...
  const connect = useCallback(async () => {
    ...
    try {
      ...
      // Small delay to let Phantom fully initialize connection
  ...
  const mockSend = useCallback(async (tokenId, amount, to) => {
    ...
    // Auto-dismiss after 3s — shows once, never reappears
    ...
    <div style={{position:"relative"}}>
      ...
      {open && (
        <div style={{position:"absolute",right:0,top:46,width:260,background:C.navyL,border:`1px solid ${C.gold}25`,borderRadius:14,padding:14,zIndex:400,boxShadow:`0 16px 48px #00000090`,animation:"fadeIn .2s ease"}}>
          ...
          <button onClick={async ()=>{await refreshBal();setOpen(false);}} style={{width:"100%",padding:"9px",borderRadius:8,background:"none",border:"none",color:C.dim2,fontSize:12,cursor:"pointer",textAlign:"left",marginBottom:2}}
...
function NotifPanel({ notifs, onClose, onRead }) {
  return(
    <div style={{
      position:"fixed", top:62, right:0, left:0, bottom:0,
      zIndex:500, display:"flex", alignItems:"flex-start", justifyContent:"flex-end",
      padding:"8px 12px 0",
      background:"transparent", pointerEvents:"none",
    }}>
      <div style={{
        width:"100%", maxWidth:320, background:C.navyL,
        border:`1px solid ${C.gold}28`, borderRadius:16, padding:"16px",
        boxShadow:`0 24px 60px #000000B0`, animation:"fadeIn .2s ease",
        pointerEvents:"all", maxHeight:"70vh", overflowY:"auto",
      }}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
          <span style={{fontSize:13,fontWeight:700,color:C.gold}}>Notifications</span>
          <button onClick={onClose} style={{background:"none",border:"none",color:C.dim,cursor:"pointer",fontSize:18,padding:"2px 6px"}}>✕</button>
        </div>
        ...
        {notifs.map(n=>(
          <div key={n.id} onClick={()=>onRead(n.id)}
            style={{display:"flex",gap:10,padding:"10px 10px",borderRadius:10,marginBottom:4,
              cursor:"pointer",background:n.read?"none":`${C.gold}08`,
              border:`1px solid ${n.read?"transparent":C.gold+"12"}`}}>
            <span style={{fontSize:20,flexShrink:0,lineHeight:1.3}}>{n.emoji}</span>
            <div style={{flex:1,minWidth:0}}>
              <div style={{fontSize:12,color:C.white,lineHeight:1.55,wordBreak:"break-word"}}>{n.msg}</div>
              <div style={{fontSize:10,color:C.dim,marginTop:3}}>{n.time}</div>
            </div>
            {!n.read&&<div style={{width:7,height:7,borderRadius:"50%",background:C.gold,flexShrink:0,marginTop:5}}/>}
          </div>
        ))}
      </div>
    </div>
      ...
      <header style={{position:"sticky",top:0,zIndex:200,background:`${C.navy}EC`,backdropFilter:"blur(20px)",borderBottom:`1px solid ${C.gold}15`,padding:"0 16px"}}>
        <div style={{maxWidth:900,margin:"0 auto",display:"flex",alignItems:"center",justifyContent:"space-between",height:58,gap:10}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
            <div style={{position:"relative"}}>
              <button onClick={()=>setShowN(!showN)} style={{width:34,height:34,borderRadius:9,background:showN?`${C.gold}20`:C.navyL,border:`1px solid ${showN?C.gold:C.gold+"22"}`,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,transition:"all .2s"}}>🔔</button>
              ...
              {showN&&<div onClick={()=>setShowN(false)} style={{position:"fixed",inset:0,zIndex:499}}/>}
            </div>
