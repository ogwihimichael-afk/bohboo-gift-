...
function usePhantom() {
  ...
  const connect = useCallback(async () => {
    ...
    try {
      ...
      const bal = await fetchBal(pk);
      const w   = { publicKey:pk, short:trim(pk), balance:bal.toFixed(4) };
  ...
  const refreshBal = useCallback(async () => {
    if (!wallet) return;
    const b = await fetchBal(wallet.publicKey);
    const w = { ...wallet, balance: b.toFixed(4) };
    setWallet(w);
    try { sessionStorage.setItem("gw", JSON.stringify(w)); } catch {}
  }, [wallet, fetchBal]);
  ...
  useEffect(() => {
    try {
      ...
      if (s) {
        ...
        setWallet(w); setWStatus("connected");
        fetchBal(w.publicKey).then(b => {
          const u = { ...w, balance: b.toFixed(4) };
          setWallet(u);
  ...
  useEffect(() => {
    ...
    const h = pk => {
      ...
      const k = pk.toString();
      fetchBal(k).then(b => {
        const w = { publicKey:k, short:trim(k), balance:b.toFixed(4) };
        setWallet(w);
        try { sessionStorage.setItem("gw", JSON.stringify(w)); } catch {}
      });
    };
  ...
  return { wallet, wStatus, wError, txStatus, lastTx, connect, disconnect, refreshBal, sendSOL };
}
...
const FEED0 = [
  {id:"f1",occasion:"Birthday",      emoji:"🎂",message:"You've carried this team for years. Today is yours. Happy birthday, legend.",from:"8xKm…3a1",to:"9bNp…7c2",amount:null,   reaction:"🥹",time:"2m ago"},
  {id:"f2",occasion:"Graduation",    emoji:"🎓",message:"First in the family to get that degree. We always knew you would.",         from:"4cZq…8d3",to:"2aWr…5e7",amount:"5 SOL",reaction:"❤️",time:"14m ago"},
  {id:"f3",occasion:"Just Because",  emoji:"💛",message:"No reason. Just wanted you to know I see how hard you work.",               from:"1fYt…2b9",to:"8cUs…4a1",amount:null,   reaction:"😭",time:"1h ago"},
  {id:"f4",occasion:"Congratulations",emoji:"🏆",message:"You built that from nothing. This is just the beginning.",                 from:"6dVs…7f4",to:"3eXp…1c6",amount:"10 SOL",reaction:"🔥",time:"3h ago"},
  {id:"f5",occasion:"Thank You",     emoji:"🙏",message:"You didn't have to help but you did. This one's from the heart.",           from:"5bOq…3d2",to:"9aLm…6f8",amount:null,   reaction:"🫶",time:"5h ago"},
];

const MEM0 = [
  {id:"m1",type:"received",occasion:"Birthday",   emoji:"🎂",message:"Happy birthday legend. Keep winning.",      from:"4cZq…8d3",amount:"2 SOL",reaction:"🥹",date:"Apr 3, 2026"},
  {id:"m2",type:"sent",    occasion:"Graduation", emoji:"🎓",message:"So proud of you. This is just the start.",  to:"9bNp…7c2",  amount:"5 SOL",date:"Mar 22, 2026"},
  {id:"m3",type:"received",occasion:"Just Because",emoji:"💛",message:"You're appreciated more than you know.",   from:"2aWr…5e7",amount:"1 SOL",reaction:"❤️",date:"Feb 14, 2026"},
];
...
// ─── ATOMS ───────────────────────────────────────────────────
const Orb = ({ s }) => (
  <div style={{position:"absolute",borderRadius:"50%",filter:"blur(90px)",opacity:.1,pointerEvents:"none",...s}}/>
);

const Pill = ({ children, c = C.gold }) => (
  <span style={{display:"inline-flex",alignItems:"center",gap:4,padding:"3px 10px",borderRadius:20,
    background:`${c}18`,border:`1px solid ${c}35`,color:c,fontSize:11,fontWeight:700,letterSpacing:.4}}>
    {children}
  </span>
);
...
const Field = ({ label, ta, ...p }) => {
  const base = {
    width:"100%", padding:"13px 16px", borderRadius:12, background:C.navyM,
    border:`1px solid ${C.gold}28`, color:C.white, fontSize:13, outline:"none",
    fontFamily: ta ? "'Playfair Display',serif" : "'DM Sans',sans-serif",
  ...
  return (
    <div>
      ...
      {ta
        ? <textarea style={base} onFocus={e=>e.target.style.borderColor=`${C.gold}70`} onBlur={e=>e.target.style.borderColor=`${C.gold}28`} {...p}/>
...
const Btn = ({ children, ghost, full, danger, disabled, onClick, style:s }) => {
  ...
  return (
    <button onClick={onClick} disabled={disabled}
      style={{padding:"13px 24px",borderRadius:12,fontSize:13,fontWeight:700,
        cursor:disabled?"not-allowed":"pointer",transition:"all .2s",
        fontFamily:"'DM Sans',sans-serif",background:bg,border,color,
...
const Toggle = ({ val, set }) => (
  <div onClick={() => set(!val)}
    ...
    <div style={{position:"absolute",top:3,left:val?22:3,width:18,height:18,
      borderRadius:"50%",background:val?C.navy:C.dim,transition:"left .25s"}}/>
  </div>
);
    ...
    <div style={{position:"relative"}}>
      <div onClick={() => setOpen(!open)}
        style={{display:"flex",alignItems:"center",gap:8,padding:"7px 12px",borderRadius:10,
          cursor:"pointer",background:`${C.green}10`,border:`1px solid ${C.green}30`,transition:"background .2s"}}
      ...
      {open && (
        <div style={{position:"absolute",right:0,top:46,width:240,background:C.navyL,
          border:`1px solid ${C.gold}25`,borderRadius:14,padding:12,zIndex:400,
          boxShadow:`0 16px 48px #00000090`,animation:"fadeIn .2s ease"}}>
          <div style={{padding:"8px 10px 12px",borderBottom:`1px solid ${C.gold}10`}}>
            ...
            <div style={{fontSize:10,color:C.white,wordBreak:"break-all",lineHeight:1.5}}>{wallet.publicKey}</div>
          </div>
          {[{label:"🔄 Refresh Balance", fn:()=>{refreshBal();setOpen(false);}}].map(({label,fn})=>(
            <button key={label} onClick={fn}
              style={{width:"100%",padding:"10px",borderRadius:8,background:"none",border:"none",color:C.dim2,fontSize:12,cursor:"pointer",textAlign:"left",marginTop:4}}
          ...
          <a href={`${EXP}/address/${wallet.publicKey}?cluster=devnet`} target="_blank" rel="noreferrer"
            onClick={()=>setOpen(false)}
            style={{display:"block",padding:"10px",borderRadius:8,color:C.dim2,fontSize:12,textDecoration:"none",marginTop:2}}
            onMouseEnter={e=>e.currentTarget.style.background=`${C.gold}10`}
            onMouseLeave={e=>e.currentTarget.style.background="none"}>🔗 View on Explorer</a>
          <button onClick={()=>{disconnect();setOpen(false);}}
            style={{width:"100%",padding:"10px",borderRadius:8,background:"none",border:"none",color:C.red,fontSize:12,cursor:"pointer",textAlign:"left",marginTop:2}}
  ...
  return (
    <div style={{position:"relative"}}>
      ...
      {wStatus === "error" && wError && (
        <div style={{position:"absolute",right:0,top:46,width:210,padding:"10px 14px",borderRadius:10,
          background:`${C.red}15`,border:`1px solid ${C.red}35`,color:C.red,fontSize:11,zIndex:300,lineHeight:1.6}}>
          {wError.includes("phantom")
            ? <span>Phantom not detected. <a href="https://phantom.app" target="_blank" rel="noreferrer" style={{color:C.gold}}>Install ↗</a></span>
            : wError}
        </div>
      )}
    </div>
...
function VoiceRecorder({ onRecorded }) {
  ...
  const start = async () => {
    try {
      ...
      M.current  = new MediaRecorder(stream);
      Ch.current = [];
    ...
    <div style={{background:C.navyM,borderRadius:14,border:`1px solid ${C.gold}25`,padding:"16px 18px"}}>
      {state === "idle" && (
        <div style={{display:"flex",alignItems:"center",gap:14}}>
          ...
          <div>
            <div style={{color:C.white,fontSize:13,fontWeight:600}}>Add a Voice Message</div>
            <div style={{color:C.dim2,fontSize:11,marginTop:2}}>Optional · Tap mic to record · Max 60s</div>
          </div>
        </div>
      )}
      {state === "recording" && (
      ...
      {(state === "recorded" || state === "playing") && (
        <div style={{display:"flex",alignItems:"center",gap:14}}>
          <button onClick={() => { if(A.current){setState("playing");A.current.play();} }}
...
function FeedCard({ item }) {
  ...
  return (
    <div style={{background:C.navyL,borderRadius:18,border:`1px solid ${C.gold}15`,padding:"20px",transition:"all .2s"}}
      onMouseEnter={e=>{e.currentTarget.style.borderColor=`${C.gold}35`;e.currentTarget.style.transform="translateY(-2px)"}}
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          {item.amount  && <Pill c={C.green}>💎 {item.amount}</Pill>}
    ...
    <div style={{position:"absolute",right:0,top:52,width:300,background:C.navyL,border:`1px solid ${C.gold}25`,borderRadius:18,padding:20,zIndex:300,boxShadow:`0 24px 60px #00000080`,animation:"fadeIn .2s ease"}}>
      ...
      {notifs.map(n => (
        <div key={n.id} onClick={() => onRead(n.id)}
          style={{display:"flex",gap:10,padding:"10px 8px",borderRadius:10,marginBottom:4,cursor:"pointer",background:n.read?"none":`${C.gold}08`}}>
          <span style={{fontSize:20,flexShrink:0}}>{n.emoji}</span>
          <div style={{flex:1}}>
            <div style={{fontSize:12,color:C.white,lineHeight:1.5}}>{n.msg}</div>
            <div style={{fontSize:10,color:C.dim,marginTop:3}}>{n.time}</div>
          </div>
...
export default function App() {
  ...
  const { wallet, wStatus, wError, txStatus, lastTx, connect, disconnect, refreshBal, sendSOL } = usePhantom();
  ...
  const [notifs,  setNotifs]  = useState([
    ...
    {id:"n3",emoji:"🤝",msg:"8xKm…3a1 added 0.5 SOL to group gift",time:"3h ago",read:true},
    {id:"n4",emoji:"⏰",msg:"Time-locked gift released",time:"1d ago",read:true},
  ]);
  const [feed,    setFeed]    = useState(FEED0);
  const [mems,    setMems]    = useState(MEM0);
  const [stats,   setStats]   = useState({gifts:12847,sol:48290,wallets:9312,claimed:94});

  // Send flow
  const [step,    setStep]    = useState(1);
  const [occ,     setOcc]     = useState(null);
  const [gift,    setGift]    = useState({to:"",sol:"",msg:"",vis:"private",lock:false,lockDate:"",contrib:[],voice:null});
  ...
  useEffect(() => {
    const t = setInterval(() => {
      ...
      setFeed(f => [{
        ...
        amount: Math.random() > .6 ? `${(Math.random()*9+1).toFixed(1)} SOL` : null,
        time:"just now",
      }, ...f.slice(0,19)]);
    }, 10000);
    return () => clearInterval(t);
  ...
  const doSend = async () => {
    setSErr("");
    if (wStatus==="connected" && wallet && gift.to && gift.sol) {
      setSending(true);
      try   { await sendSOL(gift.to, parseFloat(gift.sol)); finish(); }
      catch (e) { setSErr(e.message || "Transaction failed. Check your balance."); setSending(false); return; }
      setSending(false);
    } else finish();
  };
  ...
  const finish = () => {
    ...
    setMems(p => [{
      ...
      amount:  gift.sol ? `${gift.sol} SOL` : null, date:"Today",
    }, ...p]);
    if (gift.vis === "public") setFeed(f => [{
      ...
      to: gift.to||`${uid()}…`, amount:gift.sol?`${gift.sol} SOL`:null, reaction:null, time:"just now",
    }, ...f]);
    setNotifs(n => [{id:`n${uid()}`,emoji:"✅",msg:"Gift sent! Awaiting claim.",time:"just now",read:false}, ...n]);
  };

  const reset = () => {
    setStep(1); setOcc(null); setDone(false); setSErr("");
    setGift({to:"",sol:"",msg:"",vis:"private",lock:false,lockDate:"",contrib:[],voice:null});
  };
  ...
  const Toast = () => {
    ...
    return (
      <div style={{position:"fixed",bottom:90,left:"50%",transform:"translateX(-50%)",padding:"12px 20px",borderRadius:14,background:cfg.bg,border:`1px solid ${cfg.br}`,color:cfg.col,fontSize:12,fontWeight:700,zIndex:500,display:"flex",alignItems:"center",gap:8,animation:"fadeIn .3s ease",boxShadow:`0 8px 32px #00000060`,whiteSpace:"nowrap"}}>
        ...
        {txStatus==="confirmed" && lastTx && (
          <a href={`${EXP}/tx/${lastTx}?cluster=devnet`} target="_blank" rel="noreferrer" style={{color:C.goldL,fontSize:11,marginLeft:4}}>View ↗</a>
        )}
      </div>
    );
  };
  ...
  if (!user && auth === "landing") return (
    <div style={{minHeight:"100vh",background:C.navy,display:"flex",alignItems:"center",justifyContent:"center",position:"relative",overflow:"hidden",padding:24}}>
      ...
      <div style={{textAlign:"center",animation:"fadeUp .7s ease",maxWidth:520,position:"relative",zIndex:1}}>
        ...
        <div style={{display:"inline-block",padding:"4px 16px",borderRadius:20,background:`${C.gold}15`,border:`1px solid ${C.gold}35`,fontSize:10,color:C.gold,letterSpacing:2,textTransform:"uppercase",marginBottom:20,fontWeight:700,fontFamily:"'DM Sans',sans-serif"}}>
          Built on Solana · Powered by Love
        </div>
        <h1 style={{fontFamily:"'Playfair Display',serif",fontSize:"clamp(40px,8vw,68px)",fontWeight:800,lineHeight:1.1,marginBottom:16,background:`linear-gradient(135deg,${C.white} 50%,${C.gold})`,WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>
          BOHBOO<br/>GIFTFi
        </h1>
        <p style={{fontSize:15,color:C.dim2,lineHeight:1.8,marginBottom:36,fontFamily:"'DM Sans',sans-serif"}}>
          The first decentralized social gifting protocol on Solana.<br/>
          Send SOL with soul — voice notes, group gifts, on-chain memories.
        </p>
        ...
        <div style={{display:"flex",gap:24,justifyContent:"center",flexWrap:"wrap"}}>
          {[["12,847+","Gifts Sent"],["48,290+","SOL Gifted"],["9,312+","Wallets"]].map(([v,l])=>(
            <div key={l} style={{textAlign:"center"}}>
              <div style={{fontSize:22,fontWeight:800,color:C.gold,fontFamily:"'Playfair Display',serif"}}>{v}</div>
  ...
  if (!user) return (
    <AuthForm
      isS={auth==="signup"}
      onSwitch={() => setAuth(auth==="signup" ? "login" : "signup")}
      onSubmit={d => setUser(d)}
      onBack={() => setAuth("landing")}
    />
  );
        ...
        {tab==="home" && (
          <div style={{animation:"fadeIn .35s ease"}}>
            {wStatus !== "connected" && (
              <div style={{background:`${C.gold}08`,border:`1px solid ${C.gold}25`,borderRadius:16,padding:"16px 20px",marginBottom:24,display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:12}}>
                <div>
                  ...
                  <div style={{fontSize:12,color:C.dim2}}>Required to send real SOL gifts on Devnet</div>
                </div>
                <WalletBtn wallet={wallet} wStatus={wStatus} wError={wError} connect={connect} disconnect={disconnect} refreshBal={refreshBal}/>
              </div>
            )}
            <div style={{textAlign:"center",padding:"32px 20px 28px"}}>
              ...
              <p style={{fontSize:14,color:C.dim2,maxWidth:460,margin:"0 auto 28px",lineHeight:1.75}}>The decentralized social gifting protocol on Solana. Send SOL with soul.</p>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))",gap:14,marginBottom:36}}>
              {[
                ...
                {icon:"🤝",title:"Group Gifting",     desc:"Pool SOL from multiple contributors into one gift."},
                ...
                {icon:"🔔",title:"Claim Notifications",desc:"Know the moment your gift lands."},
                {icon:"◇", title:"On-Chain Memories", desc:"A permanent history of giving and being loved."},
              ].map((f,i) => (
                <div key={i} style={{background:C.navyL,borderRadius:14,border:`1px solid ${C.gold}12`,padding:"18px",transition:"all .2s"}}
                  onMouseEnter={e=>{e.currentTarget.style.borderColor=`${C.gold}35`;e.currentTarget.style.transform="translateY(-2px)"}}
        ...
        {tab==="send" && (
          <div style={{animation:"fadeIn .35s ease",maxWidth:560,margin:"0 auto"}}>
            ...
            <div style={{background:wStatus==="connected"?`${C.green}08`:`${C.gold}08`,border:`1px solid ${wStatus==="connected"?C.green:C.gold}25`,borderRadius:12,padding:"12px 16px",marginBottom:24,display:"flex",alignItems:"center",justifyContent:"space-between",gap:12,flexWrap:"wrap"}}>
              <div style={{fontSize:12,color:wStatus==="connected"?C.green:C.dim2}}>
                {wStatus==="connected" ? `✅ ${wallet.short} · ${wallet.balance} SOL · Devnet` : "⚠️ Connect Phantom to send real SOL"}
              </div>
            ...
            {!done ? (
              <>
                <div style={{display:"flex",gap:6,marginBottom:28}}>
                  {["Occasion","Details","Extras","Review"].map((s,i) => (
                ...
                {step===2 && (
                  <div style={{animation:"fadeIn .25s ease",display:"flex",flexDirection:"column",gap:16}}>
                    ...
                    <Field label="Amount (SOL)" value={gift.sol} type="number" placeholder="0.00" onChange={e=>setGift({...gift,sol:e.target.value})} style={{fontSize:20,fontWeight:700}}/>
                    {wStatus==="connected" && gift.sol && (
                      <div style={{fontSize:11,marginTop:-8,color:parseFloat(gift.sol)>parseFloat(wallet.balance)?C.red:C.green}}>
                        {parseFloat(gift.sol)>parseFloat(wallet.balance) ? `⚠️ Insufficient — you have ${wallet.balance} SOL` : `✓ Balance sufficient (${wallet.balance} SOL)`}
                      </div>
                    )}
                ...
                {step===4 && (
                  <div style={{animation:"fadeIn .25s ease"}}>
                    <div style={{background:C.navyL,borderRadius:20,border:`1px solid ${C.gold}25`,padding:"24px",marginBottom:20}}>
                      ...
                      {[
                        ...
                        {l:"Amount",       v:gift.sol  ? `${gift.sol} SOL` : "—"},
              ...
              <div style={{animation:"fadeIn .4s ease",textAlign:"center"}}>
                <div style={{width:80,height:80,borderRadius:"50%",margin:"0 auto 20px",background:`${C.gold}15`,border:`2px solid ${C.gold}40`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:36,animation:"float 3s ease infinite"}}>🎁</div>
                <h3 style={{fontFamily:"'Playfair Display',serif",fontSize:24,marginBottom:8}}>Gift Sent!</h3>
                <p style={{color:C.dim2,fontSize:13,marginBottom:26,lineHeight:1.7}}>Live on Solana Devnet.<br/>Share the link with your recipient.</p>
                {lastTx && (
          ...
          <div style={{animation:"fadeIn .35s ease"}}>
            <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:22,flexWrap:"wrap",gap:12}}>
              <div>
                ...
                <p style={{color:C.dim2,fontSize:13}}>A living wall of generosity on Solana.</p>
              </div>
              <div style={{display:"flex",alignItems:"center",gap:8}}>
          ...
          <div style={{animation:"fadeIn .35s ease"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:10,marginBottom:24}}>
              {[
                ...
                {v:mems.filter(m=>m.type==="received").length, l:"Gifts Received", c:C.purpL},
                {v:mems.reduce((a,m)=>a+(m.amount?parseFloat(m.amount):0),0).toFixed(1), l:"SOL Gifted", c:C.green},
              ].map(({v,l,c}) => (
                <div key={l} style={{padding:"16px",borderRadius:14,background:`${c}0E`,border:`1px solid ${c}28`,textAlign:"center"}}>
                  <div style={{fontSize:24,fontWeight:800,color:c,fontFamily:"'Playfair Display',serif"}}>{v}</div>
                  <div style={{fontSize:10,color:C.dim,letterSpacing:.8,textTransform:"uppercase",marginTop:4}}>{l}</div>
                </div>
              ))}
            </div>
            <div style={{display:"flex",flexDirection:"column",gap:12}}>
          ...
          <div style={{animation:"fadeIn .35s ease",maxWidth:480,margin:"0 auto"}}>
            ...
            {rxStep==="open" && (
              <div style={{background:C.navyL,borderRadius:24,border:`1px solid ${C.gold}30`,padding:"36px 28px",textAlign:"center",animation:"glow 3s ease infinite"}}>
                ...
                <Pill>Birthday</Pill>
                <div style={{marginTop:20,marginBottom:8,fontSize:10,color:C.dim2,letterSpacing:1.5,textTransform:"uppercase"}}>From</div>
                <div style={{fontSize:13,color:C.white,marginBottom:24,fontWeight:600}}>4cZq…8d3</div>
                <div style={{background:C.navyM,borderRadius:14,padding:"16px",marginBottom:20,border:`1px solid ${C.gold}15`,textAlign:"left"}}>
            ...
            {rxStep==="reveal" && (
              <div style={{animation:"fadeIn .4s ease",textAlign:"center"}}>
                <div style={{background:C.navyL,borderRadius:24,border:`1px solid ${C.gold}35`,padding:"40px 28px",boxShadow:`0 0 80px ${C.gold}25`}}>
                  <div style={{fontSize:60,marginBottom:16}}>💎</div>
                  <h3 style={{fontFamily:"'Playfair Display',serif",fontSize:38,color:C.gold,marginBottom:6}}>5 SOL</h3>
                  <p style={{color:C.dim2,fontSize:13,marginBottom:28}}>A birthday gift from your people.</p>
              ...
              <div style={{animation:"fadeIn .4s ease"}}>
                <div style={{background:C.navyL,borderRadius:20,border:`1px solid ${C.green}25`,padding:"20px",marginBottom:16,display:"flex",alignItems:"center",gap:14}}>
                  ...
                  <div>
                    ...
                    <div style={{fontSize:12,color:C.dim2}}>5 SOL has been sent to your wallet.</div>
                  </div>
                </div>
      ...
      <div style={{width:"100%",maxWidth:400,animation:"fadeUp .4s ease",position:"relative",zIndex:1}}>
        ...
        <div style={{background:C.navyL,borderRadius:24,border:`1px solid ${C.gold}20`,padding:"28px"}}>
          ...
          <div style={{textAlign:"center",marginTop:20,fontSize:12,color:C.dim2}}>
            ...
            <button onClick={onSwitch} style={{background:"none",border:"none",color:C.gold,cursor:"pointer",fontWeight:700,fontSize:12}}>
              {isS ? "Sign In" : "Sign Up"}
            </button>
          </div>
        </div>
